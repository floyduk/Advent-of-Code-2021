...
# Are we testing or live?
fishes = input

fish_count = [0,0,0,0,0,0,0,0,0]

# Start by counting how many fish are at each age
for fish in fishes:
    fish_count[fish] += 1
...
for age in range(256):
    # Get the number of new fish
    new_fish = fish_count[0]

    # Shuffle everything left by 1
    del fish_count[0]

    # The fish that spawned go back to age 6
    fish_count[6] += new_fish

    # Finally add the new fish
    fish_count.append(new_fish)

    print("After ", age, " days: ", fish_count)
